document.addEventListener("DOMContentLoaded", () => {
  ...
  function inserirJogo(jogo) {
    ...
    btnExcluir.addEventListener(
      "click",
      () => abrirModalExcluir(jogo.id_tabuleiro),
      false
    );
  ...
  function excluirJogo(id_tabuleiro) {
    fetch(`excluir.php?id_tabuleiro=${id_tabuleiro}`, {
      ...
      .then((retorno) => {
        if (retorno) {
          listarTodos(); // Atualiza a lista após exclusão
          M.toast({ html: 'Jogo excluído com sucesso!', classes: 'red', displayLength: 4000 });
